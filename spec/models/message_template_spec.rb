  ...
  describe "class methods" do
    describe ".existing_reasons" do
      ...
      it "指定ユーザーの理由が含まれる" do
        expect(described_class.existing_reasons(user.uuid)).to include("自分の理由")
      end

      it "共通テンプレートの理由が含まれる" do
        expect(described_class.existing_reasons(user.uuid)).to include("共通の理由")
      end

      it "他ユーザーの理由は含まれない" do
        expect(described_class.existing_reasons(user.uuid)).not_to include("他人の理由")
      end

      it "重複した理由は1件だけ返す" do
        # 同じ理由を持つテンプレートをもう1件作成する
        create(:message_template, user_uuid: user.uuid, reason: "自分の理由")
        reasons = described_class.existing_reasons(user.uuid)
